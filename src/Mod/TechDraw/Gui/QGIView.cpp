...
void QGIView::snapPosition(QPointF& mPos)
{
    ...
    if (sectionView) {
        ...
        Base::Vector2d bvPt = Base::Vector2d(bvPos.x(), bvPos.y());
        Base::Vector2d mPt = Base::Vector2d(mPos.x(), mPos.y());
        Base::Vector2d dir = Base::Vector2d(dir3d.x, dir3d.y).Normalize();
        ...
        Base::Vector2d v = ((projPt - bvPt) + 0.5 * sSize * dir) - (0.5 * bSize * dir);
       ...
       v = ((projPt - bvPt) - 0.5 * sSize * dir) + (0.5 * bSize * dir);
        sign = v * dir > 0 ? - 1 : 1;
        dist = v.Length();
        if (dist < snapDist) {
            v = dist * dir;
